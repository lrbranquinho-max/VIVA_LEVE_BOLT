...
export default function PlanosConfiguracao() {
  ...
  return <details className="border-y bg-white p-4"><summary className="cursor-pointer font-black text-viva-roxo">Configurações dos planos</summary>
    {config && <div className="mt-4 grid gap-5 md:grid-cols-3">
      ...
      <fieldset><legend className="mb-2 text-sm font-bold">Cartão Alimentação presencial</legend>{['Alelo', 'VR', 'Ticket', 'Pluxee'].map(b => <label key={b} className="mb-2 flex items-center gap-2 text-sm"><input type="checkbox" checked={Boolean(config.bandeiras[b])} onChange={e => setConfig({ ...config, bandeiras: { ...config.bandeiras, [b]: e.target.checked } })} />{b}</label>)}</fieldset>
