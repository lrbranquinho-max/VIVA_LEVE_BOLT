...
export default function AdminEntregasPage() {
  ...
  const [form, setForm] = useState({ nome: '', email: '', telefone: '', observacoes: '', senha: '', ativo: true });
  ...
  const carregar = useCallback(async () => {
    ...
    const [rolesRes, pedidosRes, historicoRes] = await Promise.all([
      ...
      supabase.from('pedidos').select('id,cliente_id,status,criado_em,updated_at,endereco_entrega,endereco,itens,pagamento_status,meio_pagamento,tipo_venda,cliente_nome_balcao,cliente_telefone_balcao,entregador_id,entrega_atribuida_em,saiu_entrega_em,entregue_em,entrega_metodo_confirmacao,entrega_observacoes,entrega_janela').order('criado_em', { ascending: false }),
  ...
  const pedidosFiltrados = useMemo(() => {
    ...
    return pedidos.filter(pedido => {
      ...
      const data = (pedido.criado_em || '').slice(0, 10);
  ...
  function abrirNovoEntregador(item?: Entregador) {
    setEditando(item ?? null);
    setForm(item ? {
      nome: item.nome, email: item.email, telefone: item.telefone || '', observacoes: item.observacoes || '', senha: '', ativo: item.ativo,
    } : { nome: '', email: '', telefone: '', observacoes: '', senha: '', ativo: true });
    setModalEntregador(true);
  }

  async function salvarEntregador(event: FormEvent) {
    event.preventDefault(); setSalvando(true);
    const { data: sessao } = await supabase.auth.getSession();
    const response = await fetch('/api/admin/entregadores', {
      method: editando ? 'PATCH' : 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${sessao.session?.access_token}` },
      body: JSON.stringify({ ...form, userId: editando?.user_id }),
    });
    const data = await response.json(); setSalvando(false);
    if (!response.ok) { avisar(data.error || 'Erro ao salvar entregador.', true); return; }
    avisar(data.conviteEnviado ? 'Entregador criado e convite enviado por e-mail.' : 'Entregador salvo com sucesso.');
    setModalEntregador(false); await carregar();
  }
    ...
    <main className="min-h-screen bg-gray-100 p-4 text-gray-900 md:p-6">
      ...
      <div className="mx-auto max-w-screen-2xl">
        <header className="mb-5 flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex gap-2"><button onClick={() => abrirNovoEntregador()} className="h-11 bg-viva-verde px-4 text-sm font-black text-viva-roxo">+ Entregador</button><Link href="/admin" className="flex h-11 items-center border border-gray-300 bg-white px-4 text-sm font-black text-viva-roxo">Voltar ao Admin</Link></div>
        </header>
      ...
      {modalEntregador && <div className="fixed inset-0 z-[80] overflow-y-auto bg-black/55 p-3 md:p-8"><form onSubmit={salvarEntregador} className="mx-auto w-full max-w-xl bg-white p-5 shadow-2xl"><h2 className="text-xl font-black">{editando ? 'Editar entregador' : 'Novo entregador'}</h2><div className="mt-4 grid gap-3 md:grid-cols-2"><label className="text-xs font-black uppercase text-gray-500">Nome<input required value={form.nome} onChange={e => setForm({ ...form, nome: e.target.value })} className="mt-1 h-12 w-full border px-3 text-sm" /></label><label className="text-xs font-black uppercase text-gray-500">Telefone<input value={form.telefone} onChange={e => setForm({ ...form, telefone: e.target.value })} className="mt-1 h-12 w-full border px-3 text-sm" /></label><label className="text-xs font-black uppercase text-gray-500 md:col-span-2">E-mail / login<input type="email" required disabled={Boolean(editando)} value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="mt-1 h-12 w-full border px-3 text-sm disabled:bg-gray-100" /></label><label className="text-xs font-black uppercase text-gray-500 md:col-span-2">{editando ? 'Nova senha (opcional)' : 'Senha (opcional; vazio envia convite)'}<input type="password" minLength={8} value={form.senha} onChange={e => setForm({ ...form, senha: e.target.value })} className="mt-1 h-12 w-full border px-3 text-sm" /></label><label className="text-xs font-black uppercase text-gray-500 md:col-span-2">Observações<textarea value={form.observacoes} onChange={e => setForm({ ...form, observacoes: e.target.value })} rows={3} className="mt-1 w-full border p-3 text-sm" /></label><label className="flex items-center gap-2 text-sm font-bold md:col-span-2"><input type="checkbox" checked={form.ativo} onChange={e => setForm({ ...form, ativo: e.target.checked })} className="h-5 w-5 accent-viva-roxo" />Acesso ativo</label></div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setModalEntregador(false)} className="h-11 border px-4 text-sm font-black">Cancelar</button><button disabled={salvando} className="h-11 bg-viva-verde px-5 text-sm font-black text-viva-roxo disabled:opacity-50">{salvando ? 'Salvando...' : 'Salvar entregador'}</button></div></form></div>}
    </main>
  );
}
