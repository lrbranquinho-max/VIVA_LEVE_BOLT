...
export default function AdminEntregasPage() {
  ...
  async function confirmarViaAdmin() {
    if (!confirmacaoAdmin) return;
    setSalvando(true);
    const { data, error } = await supabase.rpc('confirmar_entrega_pelo_admin', {
      p_pedido_id: confirmacaoAdmin.pedido.id,
      p_observacao: confirmacaoAdmin.observacao || null,
    });
    setSalvando(false);
    if (error) { avisar(`Erro ao confirmar entrega: ${error.message}`, true); return; }
    if (!data?.ok) { avisar(data?.message || 'Não foi possível confirmar a entrega.', true); return; }
    avisar('Entrega confirmada via admin.');
  ...
  return (
    <main className="min-h-screen bg-gray-100 p-4 text-gray-900 md:p-6">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          <section className="space-y-3">
            {pedidosFiltrados.map(pedido => {
              ...
              return <article key={pedido.id} className="bg-white shadow-sm"><header className="flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 p-4"><div><p className="font-mono text-xs font-bold text-gray-400">PEDIDO #{pedido.id}</p><h2 className="mt-1 text-lg font-black">{nomeCliente(pedido)}</h2><p className="text-sm text-gray-500">{telefoneCliente(pedido) || 'Telefone não informado'}</p></div><span className={`px-3 py-1 text-xs font-black uppercase ${badgeStatus(status)}`}>{nomeStatus(status)}</span></header><div className="grid gap-4 p-4 md:grid-cols-2"><div><p className="text-xs font-black uppercase text-gray-400">Endereço</p><p className="mt-1 text-sm font-bold text-gray-700">{pedido.endereco_entrega || pedido.endereco || 'Retirada / endereço não informado'}</p><p className="mt-3 text-xs font-black uppercase text-gray-400">Responsável</p><p className="mt-1 text-sm font-bold">{responsavel?.nome || 'Ainda não atribuído'}</p>{pedido.entrega_janela && <p className="text-xs text-gray-500">Janela: {pedido.entrega_janela}</p>}{pedido.entrega_observacoes && <p className="mt-2 whitespace-pre-line text-xs font-semibold text-gray-600">Observação: {pedido.entrega_observacoes}</p>}</div><div><p className="text-xs font-black uppercase text-gray-400">Pagamento</p><p className="mt-1 text-sm font-bold">{pedido.plano_id && <Link href="/admin/planos" className="block text-viva-roxo underline">Plano · Entrega {pedido.entrega_numero} · {pedido.entrega_prevista?.split('-').reverse().join('/')}</Link>}{nomeMeioPagamento(pedido.meio_pagamento)} · {pedido.pagamento_status || 'não informado'}</p><p className="mt-3 text-xs font-black uppercase text-gray-400">Itens</p><p className="mt-1 text-sm text-gray-600">{(pedido.itens || []).map(item => `${item.quantidade}x ${item.nome}`).join(' · ')}</p></div></div><footer className="flex flex-wrap gap-2 border-t border-gray-100 p-4"><button onClick={() => setAtribuicao({ pedido, entregadorId: pedido.entregador_id || '', observacoes: pedido.entrega_observacoes || '', janela: pedido.entrega_janela || '' })} disabled={status === 'entregue'} className="h-10 bg-viva-roxo px-4 text-xs font-black text-white disabled:opacity-40">{pedido.entregador_id ? 'Alterar entregador' : 'Atribuir entregador'}</button>{status === 'rota' && <button onClick={() => setConfirmacaoAdmin({ pedido, observacao: '' })} className="h-10 bg-emerald-600 px-4 text-xs font-black text-white">Confirmar entrega via admin</button>}<button onClick={() => setPedidoHistorico(pedidoHistorico === pedido.id ? null : pedido.id)} className="h-10 border border-gray-300 px-4 text-xs font-black text-gray-700">{pedidoHistorico === pedido.id ? 'Ocultar histórico' : 'Ver histórico'}</button></footer>{pedidoHistorico === pedido.id && <div className="border-t bg-gray-50 p-4"><h3 className="mb-3 text-sm font-black">Histórico da entrega</h3><div className="space-y-2">{historico.filter(item => item.pedido_id === pedido.id).map(item => <div key={item.id} className="grid grid-cols-[7rem_1fr] gap-3 text-xs"><span className="text-gray-400">{formatarData(item.criado_em)}</span><span className="font-bold text-gray-700">{item.evento.replace(/_/g, ' ')}{item.metodo_confirmacao ? ` · ${item.metodo_confirmacao.replace(/_/g, ' ')}` : ''}</span></div>)}{!historico.some(item => item.pedido_id === pedido.id) && <p className="text-xs text-gray-400">Nenhum evento logístico registrado.</p>}</div></div>}</article>;
            })}
      ...
      {confirmacaoAdmin && <div className="fixed inset-0 z-[85] flex items-end bg-black/55 md:items-center md:justify-center md:p-4"><section role="dialog" aria-modal="true" aria-label="Confirmar entrega via admin" className="w-full bg-white p-5 shadow-2xl md:max-w-lg"><h2 className="text-xl font-black">Confirmar entrega #{confirmacaoAdmin.pedido.id}</h2><p className="mt-2 text-sm text-gray-600">Use esta opção quando a entrega foi realizada, mas o cliente não informou o código. O administrador e o horário ficarão registrados no histórico.</p><div className="mt-4 rounded border-l-4 border-emerald-500 bg-emerald-50 p-3 text-sm font-bold text-emerald-800">A observação “Entrega confirmada via admin.” será adicionada automaticamente.</div><label className="mt-4 block text-xs font-black uppercase text-gray-500">Detalhes adicionais (opcional)<textarea maxLength={1000} value={confirmacaoAdmin.observacao} onChange={e => setConfirmacaoAdmin({ ...confirmacaoAdmin, observacao: e.target.value })} rows={3} placeholder="Ex.: confirmação recebida por telefone" className="mt-1 w-full border border-gray-300 p-3 text-sm" /></label><div className="mt-5 flex justify-end gap-2"><button disabled={salvando} onClick={() => setConfirmacaoAdmin(null)} className="h-11 border px-4 text-sm font-black disabled:opacity-50">Cancelar</button><button disabled={salvando} onClick={confirmarViaAdmin} className="h-11 bg-emerald-600 px-5 text-sm font-black text-white disabled:opacity-50">{salvando ? 'Confirmando...' : 'Confirmar como entregue'}</button></div></section></div>}
