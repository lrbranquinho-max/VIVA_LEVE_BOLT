...
const STATUS_ENTREGA = ['PROGRAMADA', 'ATRIBUIDA', 'EM_PREPARO', 'PRONTA', 'EM_ROTA', 'NAO_ENTREGUE', 'ENTREGUE', 'CANCELADA'];
const NOMES_STATUS: Record<string, string> = { PROGRAMADA: 'Programada', ATRIBUIDA: 'Atribuída', EM_PREPARO: 'Em preparo', PRONTA: 'Pronta', EM_ROTA: 'Em rota', NAO_ENTREGUE: 'Não entregue', ENTREGUE: 'Entregue', CANCELADA: 'Cancelada' };
function dataEntrega(p: Pedido) { return (p.entrega_prevista || p.criado_em || '').slice(0, 10); }
function statusEntrega(p: Pedido) { return p.status === 'Em Preparo' ? 'EM_PREPARO' : p.status_entrega_operacional; }
function encerrada(p: Pedido) { return ['ENTREGUE', 'CANCELADA'].includes(statusEntrega(p)); }
...
function classeStatus(status: string) {
  ...
  if (['EM_PREPARO', 'PRONTA', 'ATRIBUIDA'].includes(status)) return 'bg-blue-100 text-blue-800';
...
export default function AdminEntregasPage() {
  ...
  const [reagendamento, setReagendamento] = useState<{ pedido: Pedido; data: string; observacao: string } | null>(null);
  ...
  const filtrados = useMemo(() => pedidos.filter(p => {
    ...
    return (!termo || texto.includes(termo)) && (!filtroStatus || statusEntrega(p) === filtroStatus) && (!filtroEntregador || p.entregador_id === filtroEntregador)
  ...
  async function reagendarEntrega() { if (!reagendamento) return; setSalvando(true); const { data, error } = await supabase.rpc('reagendar_entrega_admin', { p_pedido_id: reagendamento.pedido.id, p_data: reagendamento.data, p_observacao: reagendamento.observacao || null }); setSalvando(false); if (error || !data?.ok) return avisar(error?.message || data?.message || 'Não foi possível reagendar.', true); setReagendamento(null); avisar('Entrega reagendada.'); await carregar(); }
  ...
  const Card = ({ pedido }: { pedido: Pedido }) => { const status = statusEntrega(pedido); const ent = responsavel(pedido.entregador_id); return <article className={`bg-white shadow-sm ${grupoEntrega(pedido) === 'Atrasadas' ? 'border-l-4 border-red-500' : ''}`}>
    {pedido.status_pagamento_operacional === 'PAGAMENTO_NA_ENTREGA' && <div className="bg-orange-500 px-4 py-2 text-center text-sm font-black uppercase text-white">Cobrar na entrega · {pedido.voucher_bandeira || 'Cartão Alimentação'}</div>}
  ...
  {confirmacaoAdmin && <div className="fixed inset-0 z-[90] flex items-end bg-black/55 md:items-center md:justify-center"><section className="w-full bg-white p-5 md:max-w-lg"><h2 className="text-xl font-black">Dar baixa na entrega #{confirmacaoAdmin.pedido.id}</h2><p className="mt-2 text-sm text-gray-600">Não é necessário atribuir entregador nem iniciar rota. Use também para retirada pelo cliente. A baixa, o administrador e o horário ficarão auditados.</p><textarea value={confirmacaoAdmin.observacao} onChange={e => setConfirmacaoAdmin({...confirmacaoAdmin,observacao:e.target.value})} placeholder="Ex.: cliente retirou no local (opcional)" className="mt-4 w-full border p-3"/><div className="mt-5 flex justify-end gap-2"><button onClick={() => setConfirmacaoAdmin(null)} className="h-11 border px-4 font-black">Voltar</button><button disabled={salvando} onClick={confirmarViaAdmin} className="h-11 bg-emerald-600 px-5 font-black text-white">Confirmar baixa</button></div></section></div>}
  {reagendamento && <div className="fixed inset-0 z-[92] flex items-end bg-black/55 md:items-center md:justify-center"><section className="w-full bg-white p-5 md:max-w-lg"><h2 className="text-xl font-black">Reagendar entrega #{reagendamento.pedido.id}</h2><p className="mt-2 text-sm text-gray-600">A alteração ficará registrada no histórico da entrega.</p><label className="mt-4 block text-sm font-black">Nova data<input type="date" min={dataBrasilia()} value={reagendamento.data} onChange={e => setReagendamento({...reagendamento,data:e.target.value})} className="mt-1 h-12 w-full border px-3"/></label><textarea value={reagendamento.observacao} onChange={e => setReagendamento({...reagendamento,observacao:e.target.value})} placeholder="Motivo ou observação (opcional)" className="mt-3 w-full border p-3"/><div className="mt-5 flex justify-end gap-2"><button onClick={() => setReagendamento(null)} className="h-11 border px-4 font-black">Voltar</button><button disabled={salvando || !reagendamento.data} onClick={reagendarEntrega} className="h-11 bg-viva-roxo px-5 font-black text-white disabled:opacity-50">Salvar nova data</button></div></section></div>}
  </main>;
}
